...
export function formatSmoke(result: SmokeResult): string {
  ...
  if (result.expectedStatusMet !== undefined) {
    checks.push(
      `  Status check:   ${result.expectedStatusMet ? "PASS" : "FAIL"}`
    );
  }
  if (result.expectedBodyMet !== undefined) {
    checks.push(
      `  Body check:     ${result.expectedBodyMet ? "PASS" : "FAIL"}`
    );
  }
...
export function formatThresholds(thresholds: ThresholdResult[]): string {
  ...
  for (const t of thresholds) {
    ...
    lines.push(
      `    ${t.name}: ${t.actual.toFixed(2)} (limit: ${t.threshold}) → ${status}`
    );
  }
  return lines.join("\n");
}
...
function formatSnapshot(snap: SoakSnapshot, index: number): string {
  return [
    `  [${index}] ${snap.elapsed}s — p50: ${formatMs(snap.latency.p50)} | p95: ${formatMs(snap.latency.p95)} | p99: ${formatMs(snap.latency.p99)} | ${formatNumber(Math.round(snap.throughput))} req/s | errors: ${formatPercent(snap.errorRate)}`,
  ].join("\n");
}
...
export function formatStressTest(result: StressTestResult): string {
  const lines = [
    "",
    SEPARATOR,
    `  Stress Test: ${result.method} ${result.url}`,
    SEPARATOR,
    "",
  ];

  lines.push(
    "  Conns │ p50       │ p95       │ p99       │ Req/s   │ Errors"
  );
  lines.push(
    "  ──────┼───────────┼───────────┼───────────┼─────────┼────────"
  );
  ...
  if (result.breakingConcurrency !== null) {
    lines.push(
      `  Breaking Point:     ${result.breakingConcurrency} conns`
    );
  } else {
