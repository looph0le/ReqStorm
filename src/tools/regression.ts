...
export const regressionSchema = z.object({
  ...
  baselineName: z.string().default('default').describe('Name for the baseline snapshot'),
  saveResult: z.boolean().default(true).describe('Save this run as the new baseline'),
  thresholds: z
    .object({
      maxP95Regression: z.number().default(20).describe('Max allowed p95 increase (%)'),
      maxErrorRateIncrease: z
        .number()
        .default(5)
        .describe('Max allowed error rate increase (percentage points)'),
      maxThroughputDrop: z.number().default(15).describe('Max allowed throughput decrease (%)'),
    })
    .partial()
    .default(() => ({}))
  ...
  try {
    const run = await runBenchmarkSplit({
      ...
      onTick: (elapsed, total) =>
        void reportProgress(ctx, elapsed, total, 'Regression test running'),
    });

    const current = run.steadyState ?? run.overall;
    const previous = loadBaseline(args.baselineName);

    const deltaPct = (cur: number, prev: number) => (prev > 0 ? ((cur - prev) / prev) * 100 : 0);
    const deltaPts = (cur: number, prev: number) => (prev !== undefined ? cur - prev : 0);
    ...
    const deltas = {
      ...
      throughput: {
        ...
        changePct:
          throughputBaseline !== undefined ? deltaPct(current.throughput, throughputBaseline) : 0,
      },
      errorRate: {
        current: current.errorRate,
        baseline: errorRateBaseline ?? current.errorRate,
        changePct:
          errorRateBaseline !== undefined ? deltaPct(current.errorRate, errorRateBaseline) : 0,
      },
    };
    ...
    if (args.thresholds.maxErrorRateIncrease !== undefined && errorRateBaseline !== undefined) {
      thresholdsChecked.push({
        ...
        passed:
          deltaPts(current.errorRate, errorRateBaseline) <= args.thresholds.maxErrorRateIncrease,
      });
    }
    ...
    if (args.saveResult) {
      saveResult(args.baselineName, 'benchmark', {
        url: args.url,
    ...
    return toolResult(formatRegression(result), result);
  } catch (err: any) {
    return toolError(`Regression test failed: ${err.message ?? err}`);
  }
}

function formatRegression(result: RegressionResult): string {
  const SE = '═'.repeat(50);
  const lines = ['', SE, `  Regression: ${result.method} ${result.url}`, SE, ''];
