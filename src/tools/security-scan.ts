...
export async function securityScanHandler(args: z.infer<typeof securityScanSchema>) {
  ...
  async function send(
    opts: Partial<typeof args> & { headers?: Record<string, string>; url?: string }
  ) {
    return httpRequest({
  ...
  if (checks.includes('security-headers')) {
    try {
      ...
      const presentHeaders = new Set(Object.keys(res.headers).map((k) => k.toLowerCase()));
      const missing = SECURITY_HEADERS.filter(([headerName]) => !presentHeaders.has(headerName));
    ...
    try {
      ...
      if (res.status === 405 || res.status === 400 || res.status === 404) {
        ...
        results.push({
          ...
          status: 'warn',
          detail:
            'Accepted X-HTTP-Method-Override header with 200 — HTTP method override may be enabled',
        });
      } else {
        results.push({
          name: 'method-override',
    ...
    try {
      ...
      if (res.status === 200) {
        results.push({
          ...
          status: 'warn',
          detail:
            'Accepted text/plain Content-Type JSON body — server may ignore content negotiation',
        });
      } else {
        results.push({
          name: 'content-type-mismatch',
  ...
  return toolResult(formatSecurityScan(result), result);
}
