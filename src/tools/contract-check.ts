...
export const contractCheckSchema = z.object({
  specUrl: z
    .string()
    .describe(
      "URL or absolute file path to OpenAPI 3.x spec (JSON or YAML)"
    ),
  baseUrl: z
    .string()
    .describe("Base URL of the running server to test against"),
  headers: z
  ...
  methods: z
    ...
    checks: z
      .object({
        statusCodes: z
          .boolean()
          .describe("Validate response status codes match spec"),
        schemas: z
          .boolean()
          .describe("Validate response body shape against spec schemas"),
        contentTypes: z
          .boolean()
          .describe("Validate Content-Type headers match spec"),
        requiredFields: z
          .boolean()
          .describe("Check required fields exist in responses"),
      })
      .partial()
      .default(() => ({}))
...
function pickStatusForRequest(operation: any): number | null {
  ...
  const keys = Object.keys(operation.responses).map(Number).filter((n) => !isNaN(n));
  keys.sort((a, b) => a - b);
...
export async function contractCheckHandler(
  args: z.infer<typeof contractCheckSchema>
) {
  ...
  try {
    ...
    for (const [path, methods] of Object.entries(paths)) {
      ...
      for (const [method, operation] of Object.entries(methods as Record<string, any>)) {
        if (["parameters", "summary", "description", "tags", "security", "servers"].includes(method)) {
          continue;
        }
    ...
    if (args.previousSpecUrl) {
      try {
        const prevApi = (await SwaggerParser.validate(
          args.previousSpecUrl
        )) as OpenApiDocument;
    ...
    return {
      content: [
        { type: "text" as const, text: formatContractCheck(result, errors) },
      ],
    };
  } catch (err: any) {
    return {
      content: [
        {
          type: "text" as const,
          text: `Contract check failed: ${err.message ?? err}`,
        },
      ],
      isError: true,
    };
...
function formatContractCheck(result: ContractCheckResult, errors: string[]): string {
  ...
  const lines = [
    ...
    ...(result.endpointsSkipped > 0
      ? [`  Skipped (auth):   ${result.endpointsSkipped}`]
      : []),
    "",
  ];
  ...
  for (const c of result.checks) {
    if (c.skipped) {
      lines.push(`  • ${c.method.padEnd(6)} ${c.path.padEnd(24)} skipped (${c.skipReason ?? "auth"})`);
      continue;
    }
  ...
  if (result.breakingChanges.length > 0) {
    ...
    for (const bc of result.breakingChanges) {
      lines.push(`    ${bc.type === "removed" ? "✗ Removed" : bc.type === "added" ? "+ Added" : "⚠ Changed"}  ${bc.detail}`);
    }
  }
