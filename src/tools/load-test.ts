  ...
  thresholds: z
    .object({
      ...
      maxErrorRate: z
        ...
        .max(100)
        .optional()
        .describe('Max acceptable error rate (0-100%)'),
    })
    .default({})
    .describe('Pass/fail thresholds'),
});

export async function loadTestHandler(args: z.infer<typeof loadTestSchema>, ctx: ProgressCtx = {}) {
  ...
  const result = await runBenchmarkSplit({
    ...
    onTick: (elapsed, total) => void reportProgress(ctx, elapsed, total, 'Load test running'),
  });
  ...
  return toolResult(formatLoadTest(loadResult), loadResult);
}
